...
from gan.model_svhn_mergan import MeRGAN
...
flags.DEFINE_boolean("pre_aloc_class", False, '')
flags.DEFINE_boolean("num_classes", 10, 'number of classes in SVHN')
...
flags.DEFINE_string("protos_path",
                    "protogan_v1_3_8/%s_order_%d/nb_cl_%d/dcgan_critic_1_ac_1.0_0.1/0.0002_0.5_0.999/10000/proto_static_random_%d_weight_0.000000_0.000000_squared_l2_0.010000/gan_dim_64/finetune_improved_v2_noise_0.5_exemplars_dual_use_%d",
...
def main(_):
    ...
    NUM_TRAIN_SAMPLES_PER_CLASS = NUM_TRAIN_SAMPLES_TOTAL / FLAGS.num_classes
    ...
    for category_idx in range(FLAGS.from_class_idx, FLAGS.to_class_idx + 1, FLAGS.nb_cl):
        ...
        train_y = train_one_hot_labels[train_indices_reals,
                  :(FLAGS.num_classes if FLAGS.pre_aloc_class else to_category_idx + 1)]
        test_x = raw_images_test[test_indices, :]
        test_y = test_one_hot_labels[test_indices,
                 :(FLAGS.num_classes if FLAGS.pre_aloc_class else to_category_idx + 1)]

        '''
        Check model exist
        '''
        if MeRGAN.check_model(FLAGS, to_category_idx):
            model_exist = True
        ...
        if num_old_classes > 0:  # first session or not
            if not model_exist:
                ...
                train_y_gens_one_hot = np.eye(FLAGS.num_classes if FLAGS.pre_aloc_class else num_old_classes)[
                    train_y_gens.reshape(-1)]
                ...
                train_y = np.concatenate((train_y,
                                          np.eye(FLAGS.num_classes if FLAGS.pre_aloc_class else to_category_idx + 1)[
                                              train_y_gens.reshape(-1)]))
        ...
        llgan_obj = MeRGAN(sess_gan, graph_gen, sess_fid,
                           ...
                           nb_cl=FLAGS.nb_cl,
                           num_seen_classes=(to_category_idx + 1),
                           ...
                           pre_aloc_class=FLAGS.pre_aloc_class,
        ...
        print(MeRGAN.model_dir_static(FLAGS))
