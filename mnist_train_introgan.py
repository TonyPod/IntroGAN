...
from gan.model_mnist_introgan import IntroGAN
...
def main(_):
    ...
    if FLAGS.dataset == 'mnist':
        ...
        raw_images_train, train_labels, train_one_hot_labels, order = svhn.load_data(kind='train',
                                                                                     order_idx=FLAGS.order_idx)
        raw_images_test, test_labels, test_one_hot_labels, order = svhn.load_data(kind='test',
                                                                                  order_idx=FLAGS.order_idx)
    ...
    for category_idx in range(FLAGS.from_class_idx, FLAGS.to_class_idx + 1, FLAGS.nb_cl):
        ...
        Check model exist
        '''
        if IntroGAN.check_model(FLAGS, to_category_idx):
            model_exist = True
        ...
        llgan_obj = IntroGAN(sess_gan, graph_gen, sess_fid,
        ...
        print(IntroGAN.model_dir_static(FLAGS))
