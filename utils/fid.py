...
def _get_input_layer(sess):
    """Prepares inception net for batched usage and returns pool_3 layer. """
    layername = 'FID_Inception_Net/ExpandDims:0'
    input_layer = sess.graph.get_tensor_by_name(layername)
    return input_layer
...
def calculate_fid_given_paths(paths, inception_path='utils/inception_model'):
...
def calculate_fid_given_paths_with_sess(sess, paths):
    ''' Calculates the FID of two paths. '''
    for p in paths:
        if not os.path.exists(p):
            raise RuntimeError("Invalid path: %s" % p)
