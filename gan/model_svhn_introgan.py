...
class ProtoGAN(object):
    ...
    def __init__(self, sess, graph, sess_fid, dataset, mode, batch_size, output_dim,
                 ...
                 order_idx, order, test_interval, proto_select_criterion, proto_importance, dim, rgb,
    ...
    def build_model(self):
        ...
        with tf.variable_scope("gan") as scope, self.graph.as_default():
                ...
                if self.dist_func == 'cosine':
                    self.proto_loss_fake = -tf.reduce_mean(tf.multiply(
                        tf.nn.l2_normalize(tf.gather(self.embedding_protos_center, tf.argmax(fake_labels, axis=1)),
                                           dim=-1),
                        tf.nn.l2_normalize(embedding_fake_class, dim=-1)))
                ...
                if self.dist_func == 'cosine':
                    self.proto_loss_real = -tf.reduce_mean(tf.multiply(
                        tf.nn.l2_normalize(tf.gather(self.embedding_protos_center, tf.argmax(self.real_y, axis=1)),
                                           dim=-1),
                        tf.nn.l2_normalize(embedding_real_class, dim=-1)))
            ...
            if self.finetune:
                var_list_for_finetune = [var for var in var_list if not ('offset' in var.name or 'scale' in var.name)]
                self.saver_for_finetune = tf.train.Saver(var_list=var_list_for_finetune)

            # grad-CAM
            self.grad_labels = tf.placeholder(tf.float32, shape=[None, self.nb_output])
            cost = tf.reduce_sum(tf.multiply(self.input_logits, self.grad_labels), axis=1)
            self.target_conv_layer = self.embedding_inputs
            self.target_conv_layer_grad = tf.gradients(cost, self.target_conv_layer)[0]
    ...
    def show_prototypes(self, frame, train_log_dir_for_cur_class):
        ...
        for category_idx in range(self.nb_output):
            ...
            gan.tflib.save_images.save_images(protos.reshape((protos.shape[0], 3, 32, 32)),
    ...
    def get_fid(self, train_log_dir_for_cur_class, FID_NUM=10000):
    ...
    def train(self, data_X, data_y, test_X, test_y, category_idx, model_exist=False):
        ...
        if model_exist:
            ...
            if not self.is_first_session and self.exemplars_dual_use:
                ...
                # if self.use_aug:
                #     images = all_protos_arr.reshape((-1, 3, 32, 32)).transpose((0, 2, 3, 1))
                #     seq = iaa.Sequential([
                    ...
                    for class_idx in range(self.nb_output - self.nb_cl, self.nb_output):
                        ...
                        for cluster_center in kmeans.cluster_centers_:
                            selected_idx = np.argmax(
                                np.sum(np.square(embedding_cur_class - cluster_center), axis=1))
                            while selected_idx in selected_indices:
                                selected_idx = np.random.randint(0, len(embedding_cur_class))
                            selected_indices.append(selected_idx)
                        dynamic_protos_val.append(data_X_float_cur_class[selected_indices])
        ...
        if not self.classification_only:
            FID_NUM = 10000
            cond_fid_file = os.path.join(train_log_dir_for_cur_class,
                                         'cond_fid.pkl' if FID_NUM == 10000 else 'cond_fid_%d.pkl' % FID_NUM)
            if not os.path.exists(cond_fid_file):
                history_cond_fid = dict()
                fid_vals = self.get_fid(train_log_dir_for_cur_class, FID_NUM)
    ...
    # @property
    # def model_dir(self):
    #     finetune_str = (('finetune_improved' + (
    ...
    #     mode_str += '_ac_%.1f_%.1f' % (self.protogan_scale, self.protogan_scale_g)
    #     proto_str = ('proto_static') + ('_%s' % self.proto_select_criterion) + (
    #         '_dup_%d' % self.proto_importance if not self.proto_importance == 1 else '') + ('_%d_weight_%f_%f' % (
    #     self.proto_num, self.proto_weight_real, self.proto_weight_fake)) + ('_%s' % self.dist_func) + (
    #                             '_%f' % self.gamma) + ('_margin_%.1f' % self.margin if not self.margin == 0 else '') + (
    #                     '' if self.center_type == 'rel_center' else '_%s' % self.center_type) + (
    #                     '_%d' % self.fixed_center_idx if self.center_type == 'fixed_center' else '')
    #     return os.path.join(self.result_dir,
    #                         self.dataset + ('_order_%d' % self.order_idx) + ('_gray' if not self.rgb else ''),
    #                         'nb_cl_%d' % self.nb_cl, mode_str,
    #                         str(self.adam_lr) + '_' + str(self.adam_beta1) + '_' + str(self.adam_beta2),
    #                         str(self.iters), proto_str, 'gan_dim_%d' % self.dim,
    #                         'classification_only_%s' % finetune_str if self.classification_only else finetune_str)

    @property
    def model_dir(self):
        return ProtoGAN.model_dir_static(self)
    ...
    def model_dir_static(FLAGS):
        ...
        return os.path.join(FLAGS.result_dir,
                            FLAGS.dataset + ('_order_%d' % FLAGS.order_idx) + ('_gray' if not FLAGS.rgb else '') + (
                            ...
                            str(FLAGS.iters), proto_str, 'gan_dim_%d' % FLAGS.dim,
    ...
    def model_dir_for_class_static(FLAGS, category_idx):
        return os.path.join(ProtoGAN.model_dir_static(FLAGS), 'class_%d-%d' % (1, category_idx + 1))
    ...
    def check_model(FLAGS, category_idx, step=-1):
        ...
        if step == -1:
            checkpoint_dir = os.path.join(ProtoGAN.model_dir_for_class_static(FLAGS, category_idx), "checkpoints",
                                          "final")
        else:
            checkpoint_dir = os.path.join(ProtoGAN.model_dir_for_class_static(FLAGS, category_idx), "checkpoints",
                                          str(step))
