...
def Linear(
    ...
    with tf.name_scope(name) as scope:
        ...
        if spectral_norm:
            weight = weights_spectral_norm(name, weight)
...
def CondProjection(
    ...
    with tf.name_scope(name) as scope:
        ...
        if inputs.get_shape().ndims == 2:
            result = tf.matmul(inputs, tf.nn.embedding_lookup(weight, labels))
        else:
            reshaped_inputs = tf.reshape(inputs, [-1, input_dim])
            result = tf.matmul(reshaped_inputs, tf.nn.embedding_lookup(weight, labels))
            result = tf.reshape(result, tf.pack(tf.unpack(tf.shape(inputs))[:-1] + [output_dim]))
