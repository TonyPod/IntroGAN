...
class GAN(object):
    ...
    def __init__(self, sess, graph, sess_fid, dataset_name, mode, batch_size, output_dim,
                 ...
                 dgr_ratio, order_idx, order, test_interval,
    ...
    def build_model(self):
        ...
        with tf.variable_scope("gan") as scope, self.graph.as_default():
            ...
            fixed_noise_256 = tf.constant(np.random.normal(size=(256, 100)).astype('float32'))
    ...
    def generate_image(self, frame, train_log_dir_for_cur_class):
        ...
        gan.tflib.save_images.save_images(samples.reshape((256, 1, 28, 28)),
    ...
    def get_fid(self, train_log_dir_for_cur_class):
        ...
        for i in range(0, FID_NUM, self.batch_size):
    ...
    def get_prob(self, inputs):
        with self.graph.as_default():
            prob = self.sess.run(self.pred_prob, feed_dict={self.real_data_int_S_test: inputs})
            return prob
    ...
    def solver(self, inputs, reuse, is_training):
        ...
        with tf.variable_scope('Solver') as scope:
            ...
            output = lib.ops.conv2d.Conv2D('s_1', 1, 64, 5, output, stride=2)
            output = leaky_relu(output)

            output = lib.ops.conv2d.Conv2D('s_2', 64, 2 * 64, 5, output, stride=2)
            if self.mode == 'wgan':
                output = lib.ops.batchnorm.Batchnorm('s_bn2', [0, 2, 3], output)
            output = leaky_relu(output)

            output = lib.ops.conv2d.Conv2D('s_3', 2 * 64, 4 * 64, 5, output, stride=2)
            if self.mode == 'wgan':
                output = lib.ops.batchnorm.Batchnorm('s_bn3', [0, 2, 3], output)
            output = leaky_relu(output)

            output = tf.reshape(output, [-1, 4 * 4 * 4 * 64])

            output = slim.flatten(output)
            classOutput = gan.tflib.ops.linear.Linear('s_ClassOutput', 4 * 4 * 4 * 64, self.nb_output, output)
            ...
            with self.graph.as_default():
                ...
                if self.finetune and not self.is_first_session:
                    ...
                    if self.improved_finetune:
                        ...
                        assert 's_ClassOutput.W' in input_tensor_names[0]
                        assert 's_ClassOutput.b' in input_tensor_names[1]

                        s_ClassOutput_W_name = input_tensor_names[0]
                        s_ClassOutput_b_name = input_tensor_names[1]
