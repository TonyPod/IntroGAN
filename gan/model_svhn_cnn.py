...
class ClsNet(object):
    ...
    def __init__(self, sess, graph, dataset_name, batch_size, output_dim, iters, result_dir,
                 adam_lr, adam_beta1, adam_beta2, nb_cl, nb_output, order_idx, order, test_interval, dim,
    ...
    def classifier(self, inputs, reuse, is_training):
        ...
        with tf.variable_scope('Classifier') as scope:
            ...
            output = lib.ops.conv2d.Conv2D('c_1', 3, self.dim, 5, output, stride=2)
            output = self.leaky_relu(output)

            output = lib.ops.conv2d.Conv2D('c_2', self.dim, 2 * self.dim, 5, output, stride=2)
            output = self.leaky_relu(output)

            output = lib.ops.conv2d.Conv2D('c_3', 2 * self.dim, 4 * self.dim, 5, output, stride=2)
            ...
            classOutput = gan.tflib.ops.linear.Linear('c_ClassOutput', 4 * 4 * 4 * self.dim, self.nb_output, output)
    ...
    def train(self, data_X_cumul, data_y_cumul, test_X_cumul, test_y_cumul, category_idx, model_exist=False,
              save_training_set=False, sess_fid=None):
        ...
        if save_training_set and sess_fid is not None:
    ...
    def model_dir(self):
        ...
        return os.path.join(self.result_dir, self.dataset_name + '_order_%d' % self.order_idx, 'nb_cl_%d' % self.nb_cl,
                            ...
                            str(self.iters), 'dim_%d' % self.dim, finetune_str)
